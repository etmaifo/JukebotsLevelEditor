...
class Editor:
    ...
    def highlight_tile(self, asset, mouseposition):
        ...
        xy = [pos[0]/BLOCK.width, pos[1]/BLOCK.height]
        #xy = self.convert(xy)
    ...
    def remove_asset(self, mouseposition):
        ...
        for tile in self.tile_group:
            if tile.rect.collidepoint(pos):
                ...
                tile.image = pygame.transform.smoothscale(tile.image, (BLOCK.width, BLOCK.height))
        ...
        for cell in self.object_group:
            if cell.rect.collidepoint(pos):
                ...
                cell.image = pygame.transform.smoothscale(cell.image, (BLOCK.width, BLOCK.height))
                for obj in self.objectlist:
    ...
    def draw(self):
        ...
        self.sidebar.blit(self.display_asset.image, (50, 10))

        self.icons.draw(self.sidebar)
    ...
    def create_tile_group(self):
        ...
        for y in range(width/BLOCK.height):
            for x in range(height/BLOCK.width):
                block = Block(y*BLOCK.width, x*BLOCK.height)
        ...
        for y in range(width/BLOCK.height):
            for x in range(height/BLOCK.width):
                cell = Block(y*BLOCK.width, x*BLOCK.height)
    ...
    def create_level(self, blocklist, width, height):
        ...
        raw_str = ""
        level = []
        for i in range(height/BLOCK.height):
            for j in range(width/BLOCK.width):
